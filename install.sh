...
case "$#:$*" in
  ...
  '1:--help') echo "Usage: $0 [--engine claude|codex|all]"; exit 0 ;;
  *) echo "Usage: $0 [--engine claude|codex|all]" >&2; exit 1 ;;
esac
# テスト・任意配置用。HOME / CODEX_HOME 自体は変更しない。
INSTALL_ROOT="${GSD_LITE_INSTALL_ROOT:-$HOME}"
if [ "$ENGINE" != claude ]; then
  command -v jq >/dev/null || { echo "jq is required for Codex installation" >&2; exit 1; }
fi
...
install_engine() {
  ...
  if [ "$engine" = codex ]; then
    jq '.engine = "codex"' "$REPO_DIR/templates/state.json" > "$data_dir/templates/state.json"
...
case "$ENGINE" in
  all) install_engine claude; install_engine codex ;;
