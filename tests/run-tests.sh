...
make_project(){ # make_project <dir> — 足場だけの git プロジェクトを作る
  ...
  cat > .gsd-lite/state.json <<'EOF'
{
  ...
  "turn": 0, "max_turns": 10, "retry": 0, "retry_max": 2,
...
# ---- スタブ 1: 正常系（research→plan→impl×2→verify→DONE をシミュレート）----
...
GSD_LITE_CLAUDE_BIN="$TESTROOT/bin/claude-noop" "$LOOP" > loop-out.log 2>&1
...
GSD_LITE_CLAUDE_BIN="$TESTROOT/bin/claude-blocker" "$LOOP" > loop-out.log 2>&1
